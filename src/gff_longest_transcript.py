...
def find_longest_transcript(input, output, clip_start=0, clip_end=0, clip_strand_specific=False, output_strand_specific=False):
    ...
    if clip_start > 0:
        transcripts_longest[pos_strand]["start"] = transcripts_longest[pos_strand]["start"] + clip_start
        transcripts_longest[~pos_strand]["end"] = transcripts_longest[~pos_strand]["end"] - clip_end
    if clip_end > 0:
        transcripts_longest[pos_strand]["end"] = transcripts_longest[pos_strand]["end"] - clip_end
        transcripts_longest[~pos_strand]["start"] = transcripts_longest[~pos_strand]["start"] + clip_start
